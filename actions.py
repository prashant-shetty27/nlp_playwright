...
# ==========================================
def load_auth_registry() -> dict:
    """
    Builds the auth registry from environment variables loaded via python-dotenv.
    Format: { "staging2.justdial.com": {"username": ..., "password": ...} }
    """
    registry = {}

    staging2_user = os.getenv("STAGING2_USERNAME")
    staging2_pass = os.getenv("STAGING2_PASSWORD")
    if staging2_user and staging2_pass:
        registry["staging2.justdial.com"] = {
            "username": staging2_user,
            "password": staging2_pass,
        }
    else:
        logging.warning("⚠️ STAGING2 credentials not found in environment. Proceeding without HTTP auth.")
